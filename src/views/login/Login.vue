      ...
      <div class="login-inner">
        ...
        <div class="login-header">
          ...
          <div class="login-header-title">
            <a
              href="javascript:;"
              :class="{ current: currentLogin }"
              @click.prevent="selectLoginType(true)"
              >手机登录</a
            >
            <a
              href="javascript:;"
              :class="{ current: !currentLogin }"
              @click.prevent="selectLoginType(false)"
              >密码登录</a
            >
          </div>
        </div>
        ...
        <div class="login-content">
          <form>
            <!--手机验证码登录部分-->
            <div :class="{ current: currentLogin }">
                ...
                <section class="login-verification">
                  ...
                  <div class="switch-show">
                    <img
                      ...
                      alt=""
                      width="20"
                      @click.prevent="pswSwitchShow(true)"
                    />
                    <img
                      ...
                      alt=""
                      width="20"
                      @click.prevent="pswSwitchShow(false)"
                    />
                  </div>
                </section>
                <section class="login-message">
          ...
          <button class="login-back" @click.prevent="$router.back()">
            返回
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 1.切换登录类型
    selectLoginType(flag) {
      this.currentLogin = flag;
    },
    // 2.获取验证码
    async getVerifyCode() {
      if (this.phoneRight) {
        ...
        // 2.3 请求数据接口
        let verifyData = await getValidationCode(this.phone);
    ...
    async login() {
      ...
      if (this.currentLogin) {
        ...
        // 请求数据
        let result = await phoneCodeLogin(this.phone, this.verifyCode);
        console.log(result);
        ...
        // 3.2.1 发起请求
        let result = await pswLogin(this.user_name, this.psw, this.captcha);
        console.log(result);
        if (result.success_code === 200) {
          // 4.1 保存用户信息
